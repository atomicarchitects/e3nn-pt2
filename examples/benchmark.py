...
if arg_parser.mode == "jax":   
    ...
    def TPLinear_E3X(kernel, x, y):
        return tplinear_e3x.apply(kernel, x, y)
    ...
    def TPLinear_E3NN(kernel, x_irreps_e3nn, y_irreps_e3nn):
        return tplinear_e3nn.apply(kernel, x_irreps_e3nn, y_irreps_e3nn)
...
for lmax in range(1, arg_parser.lmax+1):
    ...
    if arg_parser.mode == "torch":
        ...
        mod = e3nn_pt2.nn.TensorProductLinear(irreps_in1_pt2, irreps_in2_pt2, batch=arg_parser.batch, device='cuda')
        tp = o3.FullyConnectedTensorProduct(irreps_in1, irreps_in2, irreps_out).to(device='cuda')

        print("Done")
        
        e3x_torch_times.append(run(mod, x, y, mode=arg_parser.mode)/arg_parser.runs)
        e3nn_torch_times.append(run(tp, x_e3nn, y_e3nn, mode=arg_parser.mode)/arg_parser.runs)
    ...
    if arg_parser.mode == "jax":
        ...
        e3x_times.append(run(TPLinear_E3X, kernel, x_jax, y_jax, mode=arg_parser.mode)/arg_parser.runs)
        
        
        tplinear_e3nn = TPLineare3nn(irreps_out=irreps_in1)
        kernel = tplinear_e3nn.init(key1, x_irreps_e3nn, y_irreps_e3nn)
        
        e3nn_jax_times.append(run(TPLinear_E3NN, kernel, x_irreps_e3nn, y_irreps_e3nn, mode=arg_parser.mode)/arg_parser.runs)
...
if arg_parser.plot:
    ...
    if arg_parser.mode == "jax":
        ...
        plt.plot(range(1, arg_parser.lmax+1), e3x_torch_times, 'orange', marker="o",label="e3nn-e3x-torch-fused")
        plt.legend()
        plt.xlabel("lmax")
        plt.ylabel("Time (s)")
        plt.yscale("log", base=2)
        plt.ylim(2**-15, 2**-3)
        plt.title(f"TP({arg_parser.features}x0e + {arg_parser.features}x1e ... \otimes 1x0e + 1x1e...)\n + Linear({arg_parser.features}x0e + {arg_parser.features}x1e ...)")
        all_even = "all_even" if arg_parser.all_even else ""
        plt.savefig(f"benchmark_tplinear_batch_{arg_parser.batch}_{all_even}.png")
